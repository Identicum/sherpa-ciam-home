resource "keycloak_openid_client" "demoapp14" {
  ...
  name                            = "demoapp14"
  description                     = "[SPA_NGINX]##contact@identicum.com##Confidential Client with PKCE enabled. Multiple post_logout."
  enabled                         = true
  ...
  valid_post_logout_redirect_uris = [ "/logoutSuccess.html", "/logoutSuccess2.html" ]
  frontchannel_logout_enabled     = false
}

resource "keycloak_openid_client_optional_scopes" "demoapp14" {
  realm_id  = resource.keycloak_realm.realm.id
  client_id = keycloak_openid_client.demoapp14.id
  optional_scopes = [ ]
}

resource "keycloak_openid_client_default_scopes" "demoapp14" {
  realm_id  = resource.keycloak_realm.realm.id
  client_id = keycloak_openid_client.demoapp14.id
